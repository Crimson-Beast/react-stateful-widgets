...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    ///    filter looks through the list, item states each object, requires id to match feturedID,
    ///    then with that new array of one object use the grab name
    return programmerList.filter((item) => featuredID === item.id)[0].name
  };
